...
@Component({
  selector: 'app-materia-quimica-path',
  standalone: true,
      ...
      <main class="main-content animate-fade-in-down" *ngIf="materia() as m">
        ...
        <div class="dashboard-body" style="position: relative; overflow: hidden; min-height: 100vh;">
          ...
          <div class="physics-bg-decorations" *ngIf="!isInfiniteModeUnlocked() || activePathTab === 'path'">
            ...
            <span class="bg-deco" style="top: 16%; right: 5%; font-size: 2.1rem; transform: rotate(10deg);">NaCl</span>
            <span class="bg-deco" style="top: 30%; left: 3%; font-size: 2.1rem; transform: rotate(8deg);">CO₂</span>
            <span class="bg-deco" style="top: 42%; right: 4%; font-size: 2.6rem; transform: rotate(-8deg);">⚗</span>
            <span class="bg-deco" style="top: 54%; left: 4%; font-size: 2.2rem; transform: rotate(-10deg);">mol</span>
            <span class="bg-deco" style="top: 65%; right: 3%; font-size: 2rem; transform: rotate(12deg);">pH</span>
            <span class="bg-deco" style="top: 76%; left: 5%; font-size: 2.1rem; transform: rotate(6deg);">Na⁺</span>
            <span class="bg-deco" style="top: 88%; right: 5%; font-size: 2.6rem; transform: rotate(-14deg);">e⁻</span>
            <span class="bg-deco" style="top: 6%; right: 22%; font-size: 1.9rem; transform: rotate(-6deg);">Cl⁻</span>
            <span class="bg-deco" style="top: 94%; left: 20%; font-size: 1.9rem; transform: rotate(9deg);">O₂</span>
                  ...
                  <div class="splash-hero">
                    <div class="splash-mascot-area">
                      <video autoplay loop muted playsinline class="splash-mascot chapter-image-custom" width="290" height="290">
                        <source src="assets/images/Nuevos VideosEIlustraciones/GifsFocoWebM/focoQuimica.webm" type="video/webm" />
                      </video>
                    </div>
  ...
  styles: [`
    ...
    .physics-bg-decorations {
      ...
      background:
        radial-gradient(ellipse 70% 45% at 15% -8%, rgba(13,148,136,0.08), transparent 60%),
        radial-gradient(ellipse 70% 45% at 100% 105%, rgba(20,184,166,0.08), transparent 60%),
        radial-gradient(rgba(13, 148, 136, 0.12) 1.5px, transparent 1.5px);
      background-size: 100% 100%, 100% 100%, 32px 32px;
    }
    .bg-deco { position: absolute; color: #0d9488; opacity: 0.22; font-family: var(--font-heading, 'Nunito', sans-serif); font-weight: 800; user-select: none; }
    .bg-deco-orbit { position: absolute; top: 6%; right: -10%; width: 460px; height: 460px; border: 2px dashed rgba(13, 148, 136, 0.16); border-radius: 50%; transform: rotate(-18deg); }
    .bg-deco-orbit::before { content: ''; position: absolute; inset: 70px; border: 2px dashed rgba(13, 148, 136, 0.13); border-radius: 50%; }
    .bg-deco-orbit::after { content: ''; position: absolute; top: 50%; left: 50%; width: 12px; height: 12px; margin: -6px; background: rgba(13, 148, 136, 0.28); border-radius: 50%; }
    .bg-deco-orbit-alt { position: absolute; bottom: 4%; left: -8%; width: 320px; height: 320px; border: 2px dashed rgba(20, 184, 166, 0.15); border-radius: 50%; transform: rotate(12deg); }
    .bg-deco-orbit-alt::before { content: ''; position: absolute; inset: 48px; border: 2px dashed rgba(20, 184, 166, 0.12); border-radius: 50%; }
    .bg-deco-orbit-alt::after { content: ''; position: absolute; top: 50%; left: 50%; width: 10px; height: 10px; margin: -5px; background: rgba(20, 184, 166, 0.25); border-radius: 50%; }
    ...
    .chapter-splash.chapter-completed { border: 2px solid #0d9488 !important; box-shadow: 0 16px 40px rgba(13,148,136,0.2), inset 0 2px 4px rgba(255,255,255,0.8) !important; background: linear-gradient(135deg, #f0fdfa 0%, #ccfbf1 100%) !important; }
    .chapter-splash.chapter-completed .splash-badge { background: #0d9488 !important; }
    ...
    .splash-mascot { width: 340px; height: 340px; position: absolute; object-fit: contain; animation: mascotFloat 3.5s ease-in-out infinite; }
    ...
    .infinite-mastery-node-card {
      ...
      position: relative;
      overflow: hidden;
      box-shadow: 0 12px 30px rgba(4, 47, 46, 0.4), 0 0 20px rgba(20, 184, 166, 0.3);
      transition: all 0.3s cubic-bezier(0.34, 1.56, 0.64, 1);
    }
    .infinite-mastery-node-card:hover {
      transform: translateY(-5px) scale(1.02);
      border-color: #f59e0b;
      box-shadow: 0 16px 40px rgba(4, 47, 46, 0.6), 0 0 30px rgba(245, 158, 11, 0.4);
    }
...
export class MateriaQuimicaPathComponent implements AfterViewInit, OnDestroy {
  ...
  getChapterConnections(index: number): { d: string, color: string, dasharray?: string }[] {
    ...
    return endOffsets.map(x2 => {
      ...
      return { d, color: isCompleted ? '#0d9488' : '#e5e5e5', dasharray: isCompleted ? 'none' : '8 8' };
    });
  }
  ...
  getConnections(index: number): { d: string, color: string, dasharray?: string }[] {
    ...
    const getColor = (sourceOffset: number) => {
      ...
      return node?.status === 'completed' ? '#0d9488' : '#e5e5e5';
    };

    const getDash = (sourceOffset: number) => {
      return getColor(sourceOffset) === '#0d9488' ? 'none' : '8 8';
    };

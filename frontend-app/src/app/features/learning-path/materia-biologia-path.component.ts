...
@Component({
  selector: 'app-materia-biologia-path',
  standalone: true,
                  ...
                  <div class="splash-hero">
                    <div class="splash-mascot-area">
                      <video autoplay loop muted playsinline class="splash-mascot chapter-image-custom" width="290" height="290">
                        <source src="assets/images/Nuevos VideosEIlustraciones/GifsFocoWebM/focoBiologia.webm" type="video/webm" />
                      </video>
                    </div>
  ...
  styles: [`
    ...
    .chapter-splash.chapter-completed { border: 2px solid #047857 !important; box-shadow: 0 16px 40px rgba(6,95,70,0.2), inset 0 2px 4px rgba(255,255,255,0.8) !important; background: linear-gradient(135deg, #f0ffeb 0%, #dcfce7 100%) !important; }
    .chapter-splash.chapter-completed .splash-badge { background: #047857 !important; }
    ...
    @media (max-width: 900px) {
      ...
      /* focoBiologia.webp trae ~10-26% de relleno transparente alrededor del pulpo (medido
    ...
    .infinite-mastery-node-card {
      ...
      position: relative;
      overflow: hidden;
      box-shadow: 0 12px 30px rgba(2, 44, 34, 0.4), 0 0 20px rgba(16, 185, 129, 0.3);
      transition: all 0.3s cubic-bezier(0.34, 1.56, 0.64, 1);
    }
    .infinite-mastery-node-card:hover {
      transform: translateY(-5px) scale(1.02);
      border-color: #f59e0b;
      box-shadow: 0 16px 40px rgba(2, 44, 34, 0.6), 0 0 30px rgba(245, 158, 11, 0.4);
    }
...
export class MateriaBiologiaPathComponent implements AfterViewInit, OnDestroy {
  ...
  constructor() {
    ...
    let matId = 'ciencias-biologia';
    if (url.includes('ciencias-biologia')) matId = 'ciencias-biologia';
  ...
  getChapterConnections(index: number): { d: string, color: string, dasharray?: string }[] {
    ...
    return endOffsets.map(x2 => {
      ...
      return { d, color: isCompleted ? '#047857' : '#e5e5e5', dasharray: isCompleted ? 'none' : '8 8' };
    });
  }
  ...
  getConnections(index: number): { d: string, color: string, dasharray?: string }[] {
    ...
    const getColor = (sourceOffset: number) => {
      ...
      return node?.status === 'completed' ? '#047857' : '#e5e5e5';
    };

    const getDash = (sourceOffset: number) => {
      return getColor(sourceOffset) === '#047857' ? 'none' : '8 8';
    };

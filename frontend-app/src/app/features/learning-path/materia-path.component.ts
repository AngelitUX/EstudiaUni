...
@Component({
      ...
      <main class="main-content animate-fade-in-down" *ngIf="materia() as m">
        ...
        <div class="dashboard-body" style="position: relative; overflow: hidden; min-height: 100vh;">
          ...
          <div class="physics-bg-decorations" *ngIf="!isInfiniteModeUnlocked() || activePathTab === 'path'">
            ...
            <span class="bg-deco" style="top: 88%; right: 5%; font-size: 2.5rem; transform: rotate(-14deg);">Contexto</span>
            <span class="bg-deco" style="top: 6%; right: 22%; font-size: 1.9rem; transform: rotate(-6deg);">Tesis</span>
            <span class="bg-deco" style="top: 94%; left: 20%; font-size: 1.9rem; transform: rotate(9deg);">Antónimo</span>
            <span class="bg-deco" style="top: 22%; left: 16%; font-size: 1.8rem; transform: rotate(14deg);">Síntesis</span>
            <span class="bg-deco" style="top: 92%; right: 25%; font-size: 1.8rem; transform: rotate(7deg);">Coherencia</span>
          </div>
  ...
  styles: [`
    ...
    @media (max-width: 900px) {
      ...
      /* focoComprensionLectora.webp trae ~16% de relleno transparente arriba/abajo y ~31%
         a los costados alrededor del pulpo (medido escaneando el canal alfa de un frame).
         Como el webp está ANIMADO (el pulpo se mueve), un recorte ajustado al 100% del
         recuadro de un solo frame cortaba tentáculos en otros puntos de la animación. Se
         escala dejando un 30% de margen extra sobre ese recuadro (además de encoger el
         contenedor) para que quepa el rango de movimiento completo. */
    ...
    .infinite-mastery-node-card {
      ...
      position: relative;
      overflow: hidden;
      box-shadow: 0 12px 30px rgba(30, 58, 138, 0.4), 0 0 20px rgba(59, 130, 246, 0.3);
      transition: all 0.3s cubic-bezier(0.34, 1.56, 0.64, 1);
    }
    .infinite-mastery-node-card:hover {
      transform: translateY(-5px) scale(1.02);
      border-color: #f59e0b;
      box-shadow: 0 16px 40px rgba(30, 58, 138, 0.6), 0 0 30px rgba(245, 158, 11, 0.4);
    }
...
export class MateriaPathComponent implements AfterViewInit, OnDestroy {
  ...
  constructor() {
    ...
    // null), así que se usa un effect() en vez de leerlo una sola vez en el queryParams
    // .subscribe() de ngOnInit: si el admin todavía no se había confirmado en ese momento,
    // el modo infinito nunca se activaba aunque la URL fuera correcta.
    effect(() => {

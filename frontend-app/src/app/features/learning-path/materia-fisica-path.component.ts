...
@Component({
  selector: 'app-materia-fisica-path',
  standalone: true,
  ...
  template: `
    <div class="lp-layout">
      ...
      <aside class="sidebar">
        ...
        <nav class="sidebar-nav">
          ...
          <a class="nav-item active" routerLink="/ruta"><img decoding="async" src="assets/images/Nuevos VideosEIlustraciones/IconosAVIF/P_RutaDeAprendizaje.avif" alt="Ruta de Aprendizaje" class="nav-icon-img"/><span class="nav-text">Ruta de Aprendizaje</span></a>
          ...
          <div class="sidebar-sub-items" [class.expanded]="herramientasExpanded" [class.collapsible]="isCollapsible">
            ...
            <a class="nav-item" routerLink="/recursos"><img decoding="async" src="assets/images/Nuevos VideosEIlustraciones/IconosAVIF/P_RecursosAdicionales.avif" alt="Recursos Adicionales" class="nav-icon-img"/><span class="nav-text">Recursos Adicionales</span></a>
          </div>
          <!-- Sidebar Promo Card -->
      ...
      <div class="mobile-overlay" [class.open]="mobileOpen" (click)="mobileOpen = false">
        <div class="mobile-menu" (click)="$event.stopPropagation()">
          ...
          <nav class="sidebar-nav">
            ...
            <a class="nav-item active" routerLink="/ruta" (click)="mobileOpen=false"><img decoding="async" src="assets/images/Nuevos VideosEIlustraciones/IconosAVIF/P_RutaDeAprendizaje.avif" alt="Ruta de Aprendizaje" class="nav-icon-img"/><span class="nav-text">Ruta de Aprendizaje</span></a>
            ...
            <div class="sidebar-sub-items" [class.expanded]="herramientasExpanded" [class.collapsible]="isCollapsible">
              ...
              <a class="nav-item" routerLink="/recursos" (click)="mobileOpen=false"><img decoding="async" src="assets/images/Nuevos VideosEIlustraciones/IconosAVIF/P_RecursosAdicionales.avif" alt="Recursos Adicionales" class="nav-icon-img"/><span class="nav-text">Recursos Adicionales</span></a>
            </div>
          </nav>
      ...
      <main class="main-content animate-fade-in-down" *ngIf="materia() as m">
        ...
        <div class="dashboard-body" style="position: relative; overflow: hidden; min-height: 100vh;">
          ...
          <div *ngIf="isInfiniteModeUnlocked() && activePathTab === 'infinite'" class="embedded-mastery-wrapper">
            <app-infinite-mastery-modal [materiaId]="'fisica'" [isEmbedded]="true" (backToPath)="activePathTab = 'path'"></app-infinite-mastery-modal>
          </div>

          <!-- PHYSICS BACKGROUND DECORATIONS (soft edge motif, kept out of the content column) -->
          <div class="physics-bg-decorations" *ngIf="!isInfiniteModeUnlocked() || activePathTab === 'path'">
            ...
            <span class="bg-deco" style="top: 4%; left: 4%; font-size: 2.4rem; transform: rotate(-12deg);">E = mc²</span>
            <span class="bg-deco" style="top: 16%; right: 5%; font-size: 2.6rem; transform: rotate(10deg);">⚛</span>
            <span class="bg-deco" style="top: 30%; left: 3%; font-size: 2.1rem; transform: rotate(8deg);">F = ma</span>
            <span class="bg-deco" style="top: 42%; right: 4%; font-size: 2.9rem; transform: rotate(-8deg);">Ω</span>
            <span class="bg-deco" style="top: 54%; left: 4%; font-size: 2.2rem; transform: rotate(-10deg);">v = λf</span>
            <span class="bg-deco" style="top: 65%; right: 3%; font-size: 2.5rem; transform: rotate(12deg);">∫</span>
            <span class="bg-deco" style="top: 76%; left: 5%; font-size: 2.1rem; transform: rotate(6deg);">P = IV</span>
            <span class="bg-deco" style="top: 88%; right: 5%; font-size: 2.6rem; transform: rotate(-14deg);">λ</span>
            <span class="bg-deco" style="top: 6%; right: 22%; font-size: 1.9rem; transform: rotate(-6deg);">Δt</span>
            <span class="bg-deco" style="top: 94%; left: 20%; font-size: 1.9rem; transform: rotate(9deg);">G</span>
            <span class="bg-deco" style="top: 22%; left: 16%; font-size: 1.8rem; transform: rotate(14deg);">c</span>
                  ...
                  <div class="splash-hero">
                    <div class="splash-mascot-area">
                      <video autoplay loop muted playsinline class="splash-mascot chapter-image-custom" width="290" height="290">
                        <source src="assets/images/Nuevos VideosEIlustraciones/GifsFocoWebM/focoFisica.webm" type="video/webm" />
                      </video>
                    </div>
    ...
    <ng-container *ngIf="isPhysicsRoute()">
      ...
      <button class="sim-tab-trigger" (click)="toggleSimPanel()" [class.panel-open]="simPanelOpen" [class.expanded]="simExpanded">
        <span class="sim-tab-icon">
          <svg class="sim-tab-icon-svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.6">
      ...
      <div class="sim-drawer" [class.open]="simPanelOpen" [class.expanded]="simExpanded">
        <div class="sim-drawer-inner">
          ...
          <div class="sim-expand-tip" style="font-size: 0.85rem; color: #4b5563; text-align: center; margin-bottom: 1rem; padding: 0.6rem; background: rgba(0,0,0,0.04); border-radius: 8px; display: flex; align-items: center; justify-content: center; gap: 0.5rem; font-weight: 500; border: 1px solid rgba(0,0,0,0.05);">
  ...
  styles: [`
    ...
    /* MOBILE */
    .mobile-header { display: none; flex-direction: column; position: fixed; top: 0; left: 0; right: 0; background: #0d0f17; border-bottom: 1px solid rgba(255,255,255,0.1); z-index: 101; box-sizing: border-box; }
    ...
    .physics-bg-decorations {
      ...
      background:
        radial-gradient(ellipse 70% 45% at 15% -8%, rgba(30,58,138,0.08), transparent 60%),
        radial-gradient(ellipse 70% 45% at 100% 105%, rgba(37,99,235,0.08), transparent 60%),
        radial-gradient(rgba(30, 58, 138, 0.12) 1.5px, transparent 1.5px);
      background-size: 100% 100%, 100% 100%, 32px 32px;
    }
    .bg-deco { position: absolute; color: #1e3a8a; opacity: 0.22; font-family: var(--font-heading, 'Nunito', sans-serif); font-weight: 800; user-select: none; }
    ...
    .splash-mascot { width: 290px; height: 290px; position: absolute; object-fit: contain; border-radius: 50%; overflow: hidden; animation: mascotFloat 3.5s ease-in-out infinite; }
    .splash-mascot.mascot-bio { width: 320px; height: 320px; position: absolute; }
    ...
    @media (max-width: 900px) {
      ...
      /* focoFisica.webp trae ~8-19% de relleno transparente alrededor del pulpo (medido
    ...
    /* Oculto en desktop (donde el nombre completo en vertical ya cabe bien); solo se muestra
       en el breakpoint móvil de más abajo, reemplazando el texto largo. */
    .sim-tab-icon-svg { display: none; width: 22px; height: 22px; }
    .sim-tab-label {
      font-size: 0.70rem;
      font-weight: 800;
      text-transform: uppercase;
    ...
    @media (max-width: 860px) {
      ...
      /* El cajón ya mide 100vw en móvil sea "ampliado" o no (regla de arriba), así que
         el botón Ampliar/Reducir no tiene ningún efecto visible ahí — solo es útil en
         desktop, donde el cajón sí crece de 400px a 800px. */
      .sim-expand-btn { display: none; }
      /* El tip de abajo le dice al usuario que use el botón Ampliar, que arriba se acaba de
         ocultar en móvil — dejaba de tener sentido. */
      .sim-expand-tip { display: none !important; }
      /* La pestaña vertical con el nombre completo ("SIMULADORES INTERACTIVOS" en texto
         vertical) ocupaba mucho alto de pantalla en móvil. Se reemplaza por un ícono. */
      .sim-tab-label { display: none; }
      .sim-tab-icon-svg { display: block; }
      .sim-tab-trigger { padding: 0.85rem 0.65rem; min-width: auto; }
    }
    ...
    .infinite-mastery-node-card {
      ...
      position: relative;
      overflow: hidden;
      box-shadow: 0 12px 30px rgba(15, 23, 42, 0.4), 0 0 20px rgba(37, 99, 235, 0.3);
      transition: all 0.3s cubic-bezier(0.34, 1.56, 0.64, 1);
    }
    .infinite-mastery-node-card:hover {
      transform: translateY(-5px) scale(1.02);
      border-color: #f59e0b;
      box-shadow: 0 16px 40px rgba(15, 23, 42, 0.6), 0 0 30px rgba(245, 158, 11, 0.4);
    }
...
export class MateriaFisicaPathComponent implements AfterViewInit, OnDestroy {

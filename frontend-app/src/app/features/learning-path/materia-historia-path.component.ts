...
@Component({
      ...
      <main class="main-content animate-fade-in-down" *ngIf="materia() as m">
        ...
        <div class="dashboard-body" style="position: relative; overflow: hidden; min-height: 100vh;">
          ...
          <div class="physics-bg-decorations" *ngIf="!isInfiniteModeUnlocked() || activePathTab === 'path'">
            ...
            <span class="bg-deco" style="top: 30%; left: 3%; font-size: 2.1rem; transform: rotate(8deg);">República</span>
            <span class="bg-deco" style="top: 42%; right: 4%; font-size: 2.2rem; transform: rotate(-8deg);">1973</span>
            <span class="bg-deco" style="top: 54%; left: 4%; font-size: 2.2rem; transform: rotate(-10deg);">Independencia</span>
            <span class="bg-deco" style="top: 65%; right: 3%; font-size: 2.5rem; transform: rotate(12deg);">⚖</span>
            <span class="bg-deco" style="top: 76%; left: 5%; font-size: 2.1rem; transform: rotate(6deg);">Revolución</span>
            <span class="bg-deco" style="top: 88%; right: 5%; font-size: 2.6rem; transform: rotate(-14deg);">📜</span>
            <span class="bg-deco" style="top: 6%; right: 22%; font-size: 1.9rem; transform: rotate(-6deg);">1492</span>
            <span class="bg-deco" style="top: 94%; left: 20%; font-size: 1.9rem; transform: rotate(9deg);">Constitución</span>
  ...
  styles: [`
    ...
    .physics-bg-decorations {
      ...
      background:
        radial-gradient(ellipse 70% 45% at 15% -8%, rgba(194,65,12,0.08), transparent 60%),
        radial-gradient(ellipse 70% 45% at 100% 105%, rgba(234,88,12,0.08), transparent 60%),
        radial-gradient(rgba(194, 65, 12, 0.12) 1.5px, transparent 1.5px);
      background-size: 100% 100%, 100% 100%, 32px 32px;
    }
    .bg-deco { position: absolute; color: #c2410c; opacity: 0.22; font-family: var(--font-heading, 'Nunito', sans-serif); font-weight: 800; user-select: none; }
    .bg-deco-orbit { position: absolute; top: 6%; right: -10%; width: 460px; height: 460px; border: 2px dashed rgba(194, 65, 12, 0.16); border-radius: 50%; transform: rotate(-18deg); }
    .bg-deco-orbit::before { content: ''; position: absolute; inset: 70px; border: 2px dashed rgba(194, 65, 12, 0.13); border-radius: 50%; }
    .bg-deco-orbit::after { content: ''; position: absolute; top: 50%; left: 50%; width: 12px; height: 12px; margin: -6px; background: rgba(194, 65, 12, 0.28); border-radius: 50%; }
    .bg-deco-orbit-alt { position: absolute; bottom: 4%; left: -8%; width: 320px; height: 320px; border: 2px dashed rgba(234, 88, 12, 0.15); border-radius: 50%; transform: rotate(12deg); }
    .bg-deco-orbit-alt::before { content: ''; position: absolute; inset: 48px; border: 2px dashed rgba(234, 88, 12, 0.12); border-radius: 50%; }
    .bg-deco-orbit-alt::after { content: ''; position: absolute; top: 50%; left: 50%; width: 10px; height: 10px; margin: -5px; background: rgba(234, 88, 12, 0.25); border-radius: 50%; }
    ...
    .chapter-splash.chapter-completed { border: 2px solid #c2410c !important; box-shadow: 0 16px 40px rgba(194,65,12,0.2), inset 0 2px 4px rgba(255,255,255,0.8) !important; background: linear-gradient(135deg, #fff7ed 0%, #ffedd5 100%) !important; }
    .chapter-splash.chapter-completed .splash-badge { background: #c2410c !important; }
    ...
    @media (max-width: 900px) {
      ...
      /* focoHistoria.webp trae ~14-15% de relleno transparente alrededor del pulpo (medido
...
export class MateriaHistoriaPathComponent implements AfterViewInit, OnDestroy {
  ...
  constructor() {
    ...
    // Carga las secciones de esta materia bajo demanda (historia viene de seeds → no-op). Idempotente.
  ...
  getChapterConnections(index: number): { d: string, color: string, dasharray?: string }[] {
    ...
    return endOffsets.map(x2 => {
      ...
      return { d, color: isCompleted ? '#c2410c' : '#e5e5e5', dasharray: isCompleted ? 'none' : '8 8' };
    });
  }
  ...
  getConnections(index: number): { d: string, color: string, dasharray?: string }[] {
    ...
    const getColor = (sourceOffset: number) => {
      ...
      return node?.status === 'completed' ? '#c2410c' : '#e5e5e5';
    };

    const getDash = (sourceOffset: number) => {
      return getColor(sourceOffset) === '#c2410c' ? 'none' : '8 8';
    };

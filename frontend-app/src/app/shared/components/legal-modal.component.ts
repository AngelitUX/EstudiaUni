    ...
    <div class="legal-modal-overlay" *ngIf="type" (click)="close.emit()">
      <div class="legal-modal-content" (click)="$event.stopPropagation()">
        ...
        <div class="legal-modal-body" *ngIf="type === 'terms'">
          ...
          <p>EstudiaUni ofrece un plan gratuito y un plan de pago ("Plan Pro"), cuyos precios, moneda (pesos chilenos) y beneficios se detallan en la sección de precios de la plataforma. El Plan Pro se contrata como un <strong>pase de acceso por tiempo definido (1 mes o 1 año, según elijas), mediante un pago único</strong>: no es una suscripción con cobro periódico ni se renueva automáticamente. Al vencer el período pagado, tu cuenta vuelve al plan gratuito hasta que adquieras un nuevo pase. Los pagos son procesados por Flow, un proveedor de pagos externo, o mediante transferencia bancaria verificada manualmente por nuestro equipo; EstudiaUni no almacena los datos completos de tu tarjeta o medio de pago. Si compras un nuevo pase mientras aún tienes días vigentes de un pase anterior, los días nuevos se suman a los que te quedaban en vez de reemplazarlos. Podemos modificar los precios de los planes hacia adelante, notificándolo con antelación razonable; los cambios no afectan un pase ya comprado.</p>

          <h4>VI. Reembolsos</h4>
          <p>Al ser un pago único por un período definido, no existe una "cancelación" que gestionar: el pase simplemente expira en la fecha indicada en tu perfil y no genera ningún cargo posterior. Salvo que la ley aplicable disponga lo contrario (por ejemplo, derecho a retracto dentro del plazo legal para compras a distancia), los pagos ya realizados no son reembolsables de forma proporcional por el tiempo no utilizado.</p>
